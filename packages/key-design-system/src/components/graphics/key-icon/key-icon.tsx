...
import faStore, {
  getSvgIconFromCache,
  cacheSvgIcon,
} from '../key-font-awesome-kit/key-fa-store';

export type FaWeight =
  | 'fa-solid'
  | 'fa-regular'
  | 'fa-light'
  | 'fa-thin'
...
export class KeyIcon implements ComponentWillLoad {
  ...
  async componentDidLoad() {
    ...
    try {
      const resp = await fetch(
        `https://ka-p.fontawesome.com/releases/${faStore.config.version}/svgs/${weight}/${icon}.svg?${qs}`
      );
  ...
  render() {
    ...
    return (
      ...
      >
        <span
          class="key-icon-wrapper"
          ref={(el) => (this.iconHolderRef = el)}
          key={this.key}
        >
