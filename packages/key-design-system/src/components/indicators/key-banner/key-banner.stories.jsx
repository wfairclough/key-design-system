...
export const Banner = (args) => {
  const { actions, icon, iconWeight, customContent, ...bannerArgs } = {
    ...
    content:
      'This job has been archived. You can still find this job by search through all inactive jobs',
...
export const BannerWithActions = (args) => {
  const actions = (
    <div slot="actions">
      <key-button variant="outline" color={'critical'}>
        Delete job
      </key-button>
      <key-button variant="outline" color="banner-default">
        See job details
      </key-button>
    </div>
  );
